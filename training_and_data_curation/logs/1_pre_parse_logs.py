...
1_pre_parse_logs.py
-------------------
Concatenates all log files into one, removing timestamps and lines
with .png  and adding separators between logs for readability.
Further parsing possible with script 2_analyze_logs.py which summarizes into .md file.
...
def find_log_files(root: Path) -> list[Path]:
    return [f for f in root.rglob("SVR_interval.log") if not any(part.startswith(".") for part in f.relative_to(root).parts)]
