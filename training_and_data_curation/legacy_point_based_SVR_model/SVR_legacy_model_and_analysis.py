...
2026-04-16 latest edits; Alexander Minidis

How to run: e.g.
uv run SVR_legacy_model_and_analysis.py --compartment soil --data_source combined

for batch execution, see `SVR_legacy_run_all_combos.sh`
"""
...
from src.legacy.chemistry_analysis import analyse_chemistry_all
...
def main(compartment: str, data_source: str):
    # Validation
    if (data_source == "vega" or data_source == "combined") and compartment == "air":
    ...
    log_file = str(logs_dir / f"SVR.log")
    ...
    ad_results = analyse_coverage_domain(compartment, reduced_artifacts, logs_dir, log_file)

    # Chemistry analysis (structuralanalysis.md sections 1-4)
    reduced_artifacts["ad_results"] = ad_results
    reduced_artifacts["importances"] = importances
    analyse_chemistry_all(compartment, reduced_artifacts, logs_dir, log_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Data source: hsbd, vega, or combined (default: hsbd)",
    )
