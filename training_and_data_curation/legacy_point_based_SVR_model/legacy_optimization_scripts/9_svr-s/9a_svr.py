...
2025-11-05 first version; Alexander Minidis
 ...
 -added pca anlaysis of chemical space coverage for SVR model
 -added applicability domain analysis
...
def main(compartment: str, data_source: str):
    ...
    # some model settings (more further below)
    target_column = "T_half_days"
    use_outlier_removal = True
    # model path
    models_dir = working_dir / "models"
    models_dir.mkdir(exist_ok=True)
    model_file = models_dir / f"SVR_{compartment}_{data_source}.joblib"
    ...
    # Extract SMILES aligned with the split (index matches X_train / X_test)
    ...
    # structural chemical space visualization coloured by AD membership
    ...
    # Optional: Print a single residuals along with the feature value
    # log_to_file(f"\nResiduals vs fr_term_acetylene values:", log_file)
    # for val, res in zip(X_test["fr_term_acetylene"], residuals):
    #     log_to_file(f"fr_term_acetylene: {val}, residual: {res}", log_file)

    # count compounds containing S in train and test sets
    def count_elements_compounds(smiles_series, elements):
        count = sum(1 for e in smiles_series if any(elem in e for elem in elements))
        return count

    elements = ["S", "N", "Cl", "Br", "F"]
    log_section("Elemental count in xtrain and xtest", log_file)
    for elem in elements:
        train_count = count_elements_compounds(smiles_train, [elem])
        test_count = count_elements_compounds(smiles_test, [elem])
        log_to_file(
            f"Number of compounds containing {elem} in train set: {train_count} out of {len(smiles_train)}",
            log_file,
        )
        log_to_file(
            f"Number of compounds containing {elem} in test set: {test_count} out of {len(smiles_test)}",
            log_file,
        )
