...
# print(df_with_descriptors)


def _smiles_standardizer_msc_old_pd(smiles: str) -> List[str]:
    """Original smiles standardizer as used in MistraSafeChem (MSC) project

    Converted to function with single SMILES due to pipeline needs.
    DF for single smiles might be a bit overkill but didn't want to change the original function too much.
...
def smiles_standardizer_msc(smiles: str) -> List[str]:
    """Smiles standardizer as used in MistraSafeChem (MSC) project
    Adapted for single SMILES input, keeping the same DF structure for consistency
    with the original function, even though it might be a bit overkill.

    Adaptend to newer pandas versions to avoid SettingWithCopyWarning.

    Input: single SMILES string

    Output: list with single standardized SMILES string (to keep the same output format as original function, which was designed for multiple SMILES)
    """
